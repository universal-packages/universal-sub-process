...
import { ExecEngine, Status, SubProcess } from '../src'

describe(ExecEngine, (): void => {
  it('runs a given command', async (): Promise<void> => {
    const subProcess = new SubProcess({ engine: 'exec', command: 'ls', args: ['-h'] })
...
tsconfig.json\n`)
  ...
  it('is prepared for when a process fails', async (): Promise<void> => {
    const subProcess = new SubProcess({ engine: 'exec', command: 'git', args: ['clone', 'nonexistent'] })
    const listener = jest.fn()
  ...
  it('is prepared for when a process is killed', async (): Promise<void> => {
    const subProcess = new SubProcess({ engine: 'exec', command: 'sleep', args: ['100'] })
    const listener = jest.fn()
  ...
  it('is receives the input correctly', async (): Promise<void> => {
    const subProcess = new SubProcess({ engine: 'exec', command: 'read variable && echo $variable', input: 'value' })
    const listener = jest.fn()
  ...
  it('runs on a given working directory', async (): Promise<void> => {
    const subProcess = new SubProcess({ engine: 'exec', command: 'ls', workingDirectory: './src' })
    const listener = jest.fn()
