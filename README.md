...
The `Orchestration` class manages multiple SubProcess instances concurrently, providing advanced process coordination, concurrency control, and inter-process communication capabilities.

```ts
import { Orchestration } from '@universal-packages/sub-process'
...
// Add processes to the orchestration
orchestration.addProcess({
  command: 'npm run test',
  workingDirectory: './packages/package-a'
})

orchestration.addProcess({
  command: 'npm run build',
  workingDirectory: './packages/package-b'
})
...
```ts
new Orchestration(options?: OrchestrationOptions)
```

#### OrchestrationOptions

Extends [EventEmitterOptions](https://github.com/universal-packages/universal-event-emitter) with the following options:

- **`processes`** `SubProcessOptions[]` (optional)
  Initial process definitions to add to the orchestration. These processes will be queued and executed when `run()` is called.

- **`maxConcurrency`** `number` (default: `os.cpus().length - 1`)
  Maximum number of processes to run concurrently. Must be at least 1.

- **`stopOnFailure`** `boolean` (default: `false`)
  Whether to stop all processes if one fails. When `true`, all running processes will be killed and no new processes will be started if any process fails.

### Instance Methods

#### **`addProcess(processOptions: SubProcessOptions)`**

Adds a new process to the orchestration queue. The process will be started automatically when concurrency allows.

```ts
orchestration.addProcess({
  command: 'npm run test',
  workingDirectory: './my-package'
})
```

#### **`run()`** **`async`**

Starts executing all queued processes respecting the concurrency limit. Returns a Promise that resolves when all processes have completed.
...
Stops all running processes gracefully and prevents new processes from starting.

```ts
await orchestration.stop()
```

#### **`sendMessage(data: Record<string, any>, processIndex?: number)`**

Sends a message to running child processes. If `processIndex` is provided, the message is sent only to that specific process.

```ts
// Send to all running processes
orchestration.sendMessage({ type: 'status-check' })

// Send to specific process
orchestration.sendMessage({ type: 'custom-command', payload: 'data' }, 1)
```

### Static Methods

#### **`Orchestration.sendMessageToParent(data: Record<string, any>)`**

Used within child processes to send messages back to the parent orchestration.
...
#### **`Orchestration.onParentMessage(callback: (data: Record<string, any>) => void)`**

Sets up a listener for messages from the parent orchestration. Returns an unsubscribe function.
...
Closes the communication channel with the parent process. This should be called when the child process is done with the parent process.
...
### Process Index Management

Each process managed by the orchestration receives a unique `PROCESS_INDEX` environment variable (1-based). This index is reused when processes complete, allowing for efficient resource management.

```ts
// Access process index within a child process
const processIndex = process.env.PROCESS_INDEX
console.log(`Running as process ${processIndex}`)
```

### Events

The Orchestration class emits events for all process lifecycle events, prefixed with `process:`:

#### **`process:running`**: Emitted when a process starts running

```ts
orchestration.on('process:running', (event) => {
  console.log(`Process ${event.subProcess.processIndex} started`)
})
```

#### **`process:succeeded`**: Emitted when a process completes successfully

```ts
orchestration.on('process:succeeded', (event) => {
  console.log(`Process ${event.subProcess.processIndex} succeeded`)
})
```

#### **`process:failed`**: Emitted when a process fails

```ts
orchestration.on('process:failed', (event) => {
  console.log(`Process ${event.subProcess.processIndex} failed`)
})
```

#### **`process:stopped`**: Emitted when a process is stopped

```ts
orchestration.on('process:stopped', (event) => {
  console.log(`Process ${event.subProcess.processIndex} was stopped`)
})
```

#### **`process:message`**: Emitted when a child process sends a message

```ts
orchestration.on('process:message', (event) => {
  console.log(`Message from process ${event.subProcess.processIndex}:`, event.data)
})
```

#### **`process:stdout`** / **`process:stderr`**: Emitted for process output

```ts
orchestration.on('process:stdout', (event) => {
  console.log(`Process ${event.subProcess.processIndex} output:`, event.payload.data)
})
```
...
const commands = ['npm run test', 'npm run lint', 'npm run build', 'npm run deploy']

for (const command of commands) {
  orchestration.addProcess({ command })
}
...
// Add 6 processes - they'll run in batches of 2
for (let i = 1; i <= 6; i++) {
  orchestration.addProcess({
    command: 'sleep',
    args: ['2'],
...
orchestration.on('process:running', () => {
  runningCount++
  console.log(`Running processes: ${runningCount}`)
})
...
const orchestration = new Orchestration({
  maxConcurrency: 3,
  stopOnFailure: true // Stop all if one fails
})

orchestration.addProcess({ command: 'npm run test' })
orchestration.addProcess({ command: 'npm run build' })
orchestration.addProcess({ command: 'some-failing-command' })
orchestration.addProcess({ command: 'npm run deploy' }) // Won't run if previous fails

try {
  await orchestration.run()
} catch (error) {
  console.log('Orchestration stopped due to failure')
}
```
...
// Add processes with monitoring
orchestration.addProcess({
  command: 'npm run test',
  workingDirectory: './package-a'
})

orchestration.addProcess({
  command: 'npm run build',
  workingDirectory: './package-b'
})
...
orchestration.on('process:stdout', (event) => {
  console.log(`📤 Process ${event.subProcess.processIndex}: ${event.payload.data}`)
})
...
orchestration.addProcess({ command: 'npm run test' })
orchestration.addProcess({ command: 'npm run lint' })
...
setTimeout(() => {
  orchestration.addProcess({ command: 'npm run build' })
  orchestration.addProcess({ command: 'npm run deploy' })
}, 1000)
